...
const QuestionCell = ({
  ...
  const sendResponse = () => {
    ...
    switch (respFromModel) {
      case "un":
        respFromModelInt = shuffledAnswers[0]?.toString();
        break;
      case "deux":
        respFromModelInt = shuffledAnswers[1]?.toString();
        break;
      case "trois":
        respFromModelInt = shuffledAnswers[2]?.toString();
        break;
      case "quatre":
        respFromModelInt = shuffledAnswers[3]?.toString();
        break;
      case "oui":
        respFromModelInt = "oui";
        break;
      case "non":
        respFromModelInt = "non";
        break;
      default:
        respFromModelInt = "Réponse par défaut";
    }

    respFromModelInt === undefined
      ? (respFromModelInt = "Réponse par défaut")
      : respFromModelInt;
    ...
    const newResponse: UsersResponse = {
      ...
      is_correct:
        respFromModelInt.toLowerCase() ===
        questionObject.good_answer.toLowerCase(),
  ...
  return (
    ...
    >
      <div className="flex flex-col">
        ...
        <h2 className="mb-4 font-bold text-3xl underline">
          Question {step} sur 10
        </h2>
        <h1 className="justify-content mb-10 text-lg relative">
          {questionObject.question}
        </h1>
      </div>
      <div className="grid grid-cols-1 items-center text-center">
        {shuffledAnswers.map((answer, index) => (
          <div className="px-4" key={index}>
            <QuestionButton
              text={
                answer.toLowerCase() == "oui" || answer.toLowerCase() == "non"
                  ? answer
                  : index + 1 + " - " + answer
              }
              onClick={() => validateResponse(answer)}
            />
          </div>
        ))}
      </div>
